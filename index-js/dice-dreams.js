...
const url = 'https://techyhigher.com/dice-dreams-free-rolls/';
const currentDate = getCurrentDate();
const dir = 'links-json';
const filePath = path.join(dir, 'dice-dreams.json');
const htmlFilePath = path.join('_includes', 'dice-dreams.html');
...
async function main() {
  try {
    ...
    $('a[href*="rewards.dicedreams.com"], a[href*="d10x.co"]').each((index, element) => {
    ...
    combinedLinks.forEach(link => {
      ...
      htmlContent += `    <span>Dice Dreams Free Rolls ${formattedDate}</span>\n`; // Custom text with formatted date
