...
const htmlFilePath = path.join('_includes', 'huuuge-casino.html');
...
async function main() {
  try {
    ...
    $('a[href*="2rc7.app.link"], a[href*="huuugecasino.onelink.me"]').each((index, element) => {
    ...
    combinedLinks.forEach(link => {
      ...
      htmlContent += `    <span>Huuuge Casino Free Chips ${formattedDate}</span>\n`; // Custom text with formatted date
