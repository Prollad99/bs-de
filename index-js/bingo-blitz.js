...
const htmlFilePath = path.join('_includes', 'bingo-blitz.html');
...
async function main() {
  try {
    ...
    $('a[href*="d10x.co"], a[href*="bingo-app-dsa.playtika.com"], a[href*="link.bingoblitz.com"], a[href*="gnocchi-www.buffalo-ggn.net"]').each((index, element) => {
    ...
    combinedLinks.forEach(link => {
      ...
      htmlContent += `    <span>Bingo Blitz Free Credits ${formattedDate}</span>\n`; // Custom text with formatted date
